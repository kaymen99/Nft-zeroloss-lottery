...
async function mintAndApproveDai(
  account,
  daiAddress,
  amount,
  spender,
...
async function fundPoolWithADAI(account, poolAddress, aDaiAddress) {
  ...
  const mint_tx = await aDai
    .connect(account)
    .mint(account.address, getAmountInWei(100000));
  await mint_tx.wait(1);

  const tx = await aDai
    .connect(account)
    .transfer(poolAddress, getAmountInWei(100000));
  await tx.wait(1);
}
