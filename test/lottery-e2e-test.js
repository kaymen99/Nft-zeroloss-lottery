...
!developmentChains.includes(network.name)
    ...
    : describe("NFT Lottery End2End Tests", () => {
        ...
        it("should allow users to enter the lottery", async () => {
            ...
            expect(participantsList).to.have.members([user1.address, user2.address, user3.address]);

            const aDai = await ethers.getContractAt("IERC20Mock", aDaiTokenAddress)
            const lotteryaDAIBalance = await aDai.balanceOf(lotteryContract.address)
            expect(getAmountFromWei(lotteryaDAIBalance)).to.be.equal(3 * getAmountFromWei(entryCost));

            const lotteryDAIBalance = await lotteryContract.lotteryDAIBalance()
            expect(getAmountFromWei(lotteryDAIBalance)).to.be.equal(3 * getAmountFromWei(entryCost));
        });
        let closetxReceipt;
            ...
            await new Promise(async (resolve, reject) => {
                lotteryContract.once('Lottery__WinnersPicked', async (winners, reward, timestamp, winnerPickedEvent) => {
                    try {
                        ...
                        expect(await nftContract.balanceOf(winners[0])).to.be.equal(lotteryReward)
                        expect(await nftContract.balanceOf(winners[1])).to.be.equal(lotteryReward)
                        ...
                        expect(_participantsList.length).to.be.equal(participantsList.length - winners.length);
